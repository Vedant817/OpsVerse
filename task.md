...
- Added persisted incident status lifecycle updates. When Supabase persistence is configured, both JSON and SSE swarm routes update the incident row to `running` before orchestration and then to `completed` or `failed` after the final package is built.
